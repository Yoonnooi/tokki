...
let skateboardSpeed = 0.2;
let skateboardModel = null;
let pickup = false;
let isRunning = false;
let sittingAnimationDuration = 70;
let skatejumpAnimationDuration = 200;
let npcMixer;
let npc2Mixer;
let npc3Mixer;
let npc4Mixer;
  ...
  loader.load(modelPath, (gltf) => {
    ...
    if (model) {
      ...
      if (modelPath === models[color]+'sittingarmdown.glb') {
        setTimeout(() => {
          loadCharacter(models[color]+'keepsittinogarmdown.glb');
        }, sittingAnimationDuration * 1000 / 60); // 프레임당 60프레임을 기준으로 계산
      }

      // if (modelPath === '/src/skatejump.glb') {
      //   setTimeout(() => {
      //     loadCharacter('/src/skate.glb');
      //   }, skatejumpAnimationDuration * 1000 / 60); // 프레임당 60프레임을 기준으로 계산
      // }
